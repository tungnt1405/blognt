<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('title.profile.sub_two_authen') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('title.profile.title_sesion1') }}
                @else
                    {{ __('title.profile.title_session2') }}
                @endif
            @else
                {{ __('title.profile.two_authen.title') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('title.profile.two_authen.description') }}
            </p>
        </div>
